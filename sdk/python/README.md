...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `pnap` with the `NAME` of your provider.
   - find and replace all instances of the boilerplate `phoenixnap` with the `ORG` of your provider.
   - replace all instances of the `github.com/phoenixnap/pulumi-pnap` repository with the `REPOSITORY` location
...
- binary: pulumi-resource-pnap
  ...
  ignore: []
  ldflags:
  - -X github.com/phoenixnap/pulumi-pnap/provider/pkg/version.Version={{.Tag}}
  main: ./cmd/pulumi-resource-pnap/
  sort: asc
  use: git
release:
  disable: false
...
├── config.yaml
├── metadata.json
├── pulumi-pnap_darwin_amd64_v1
│   └── pulumi-resource-pnap
├── pulumi-pnap_darwin_arm64
│   └── pulumi-resource-pnap
├── pulumi-pnap_linux_amd64_v1
│   └── pulumi-resource-pnap
├── pulumi-pnap_linux_arm64
│   └── pulumi-resource-pnap
├── pulumi-pnap_windows_amd64_v1
│   └── pulumi-resource-pnap.exe
└── pulumi-pnap_windows_arm64
    └── pulumi-resource-pnap.exe
```
